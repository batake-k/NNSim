...
po::options_description DefineCalcOption() {
  ...
  opt.add_options()
    ...
    ("A,a", po::value<float>()->default_value(1), "Constraint on the number of times each polyomino is used")
    ("B,b", po::value<float>()->default_value(1), "Constraint on the overlap of the polyominoes on the board")
    ("C,c", po::value<float>()->default_value(1), "Inhibition of bubbles")
    ("D,d", po::value<float>()->default_value(1), "Encouragement for combination of polyominoes which contact each other")
    ("E,e", po::value<float>()->default_value(1), "Encouragement for polyominoes which contact borders of the board")
    ("F,f", po::value<float>()->default_value(1), "Broad restriction on all pairs of neurons")
    ("G,g", po::value<float>()->default_value(1), "Restriction on the size of polyominoes")
    ("cut_bubble_size,s", po::value<int>(), "maximum bubble size")
    ("output_data,x", po::value<bool>()->default_value(true), "flag to output data file")
