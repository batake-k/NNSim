...
void addSurroundingHex(const int x, const int y, vector<Hex> &surrounding_hex, const vector<Hex> &hexs,
                       const vector<Hex> &board = vector<Hex>()) {
  vector<Hex> possible_hexs{Hex(0, x + 1, y + 1), Hex(0, x + 2, y), Hex(0, x + 1, y - 1),
                            Hex(0, x - 1, y - 1), Hex(0, x - 2, y), Hex(0, x - 1, y + 1)};
